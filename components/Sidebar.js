...
import sidebarTop from "../img/sidebartop.webp";
function Sidebar() {
    ...
    <div className="space-y-2 min-w-max max-w-lg">
      ...
      <div className="bg-white dark:bg-[#1D2226] rounded-lg overflow-hidden relative flex flex-col items-center text-center border border-gray-300 dark:border-none">
        <div className="relative w-full h-14">
          <Image src={sidebarTop} layout="fill" alt="image" priority />
        </div>
        {
            /* Avatar */
        }
        {/* <div className="absolute top-8 cursor-pointer" onClick={signOut}>
          <div className="relative h-14 w-14 rounded-full border-2">
            <Image
        ...
        <div className="mt-5 py-4 space-x-0.5">
          <h4 className="hover:underline decoration-purple-400 underline-offset-1 cursor-pointer">
            {session?.user?.name}
          </h4>
      ...
      <div className="hidden md:flex bg-white dark:bg-[#1d2226] text-black/70 dark:text-white/75 rounded-lg overflow-hidden flex-col space-y-2 pt-2.5 sticky top-20 border border-gray-300 dark:border-none">
        ...
        <div className="sidebarButton text-center">
          <h4 className="dark:to-white font-medium text-sm">Discover More</h4>
        </div>
      </div>
    </div>
  );
